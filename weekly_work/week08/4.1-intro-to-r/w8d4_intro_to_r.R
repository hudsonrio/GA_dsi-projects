...
m
apply(m,1,max) # apply functions across matrices/vectors - row axis
apply(m,2,max) # apply has several variants - column axis
...
## factors
f <- factor(c('red','red','red','blue','red','green','green')) # same as set function
f
...
ggplot(data=iris, aes(x=Species, y=Petal.Length)) + geom_boxplot() + 
  scale_y_continuous(breaks = c(2,3,4,7)) +
