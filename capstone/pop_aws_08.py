...
def pop_clean_analyze(pop_10, pop_10_age, df_name, col, year):
    ...
    df_name['pop_female'] = df_name['population_age_sex'] * df_name['sex'].apply(lambda x: 1 if x == 1 else 0)
...
pop_08['STATE'] = pop_08['STATE'].apply(lambda x: state_fix(x))
pop_08['COUNTY'] = pop_08['COUNTY'].apply(lambda x: county_fix(x))
pop_08['state_code'] = pop_08['STATE'] + pop_08['COUNTY']
pop_08['county_code'] = pop_08['state_code'].apply(lambda x: int(x))
...
pop_08_g.reset_index(inplace=1)
pop_08_g.to_csv('pop_08_output.csv')
